"""
v2 download: pre-filter titles for junk BEFORE downloading (saves bandwidth),
then download/resize/hash-dedupe/license-check exactly like v1.
"""
import json, os, hashlib, urllib.request, time, re
...
seen_this_round = set()
accepted = []
rejected = {"title": 0, "license": 0, "dup": 0, "download_fail": 0, "too_small": 0}
...
print(f"\nDONE. scanned={len(candidates)} accepted={len(accepted)} rejected={rejected}")
with open(f"{EXP}/scratch/staged_accepted2.json", "w", encoding="utf-8") as f:
    json.dump(accepted, f, indent=1, ensure_ascii=False)
print("saved staged_accepted2.json")
