...
with open(f"{EXP}/scratch/candidates_raw3_damp.json", encoding="utf-8") as f:
...
print(f"already have {len(already)} accepted; skipping their titles/hashes")

BAD_TITLE_PATTERNS = [
    r"\bmicroform\b", r"\bstudy\b", r"commonwealth", r"\bmanual\b", r"hurricane",
    r"saturn", r"reservoir", r"\bmuseum\b(?!.*track)", r"citroen 7 cv",
    r"\bbook\b", r"catalogue", r"newspaper", r"gazette", r"\bmap\b",
    r"floor ?plan", r"blueprint", r"logo\b", r"trophy", r"podium only",
    r"portrait", r"headshot", r"\btrack layout\b", r"circuit map",
]
...
with open(f"{EXP}/scratch/staged_accepted4.json", "w", encoding="utf-8") as f:
    json.dump(accepted, f, indent=1, ensure_ascii=False)
print("saved staged_accepted4.json", flush=True)
