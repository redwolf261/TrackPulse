"""
Export exp03 fine-tuned checkpoint to ONNX (static 1x3x224x224), saved as
models/trackpulse_classifier_v4_exp03.onnx (does NOT touch models/trackpulse_classifier.onnx
or v2/v3 files). Then verify numerical equivalence vs PyTorch on the 49-image
racing eval set, the 67-image expanded eval set, and RSCD 305-image test set.
"""
import os, json
import numpy as np
import torch
...
ONNX_OUT = f'{ROOT}/models/trackpulse_classifier_v4_exp03.onnx'
...
with open(f'{ROOT}/data/racing_spotcheck/ground_truth_manifest.json', encoding='utf-8') as f:
    gt49 = json.load(f)
racing_paths_49 = [e['filepath'] for e in gt49]

with open(f'{ROOT}/data/racing_spotcheck_v2/ground_truth_manifest.json', encoding='utf-8') as f:
    gt67 = json.load(f)
racing_paths_67 = [e['filepath'] for e in gt67]

test_df = pd.read_csv(f'{ROOT}/data/manifests/split_manifest_test.csv')
rscd_paths = test_df['filepath'].tolist()

TOLERANCE_NOTE = "prediction_mismatches counted where argmax(pt_probs) != argmax(onnx_probs); max_prob_diff is max abs diff in softmax probability across all classes/images"

for name, paths in [('racing_eval_49', racing_paths_49), ('racing_eval_67', racing_paths_67), ('rscd_test_305', rscd_paths)]:
...
results['tolerance_note'] = TOLERANCE_NOTE
