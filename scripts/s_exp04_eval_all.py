"""
exp04 evaluation: run a fine-tuned checkpoint (PyTorch) on:
  (a) the untouched 49-image racing eval set
  (b) the 67-image expanded eval set (49 + 18 held-out DAMP)
  (c) the RSCD 305-image held-out test set
Same protocol as exp01/exp02/exp03's evaluation for direct comparability.

Usage: python s_exp04_eval_all.py optionA
"""
import os, sys, json
import numpy as np
import torch
...
CKPT = f'{ROOT}/experiments/exp04_damp_rebalance/checkpoints_{OPTION}/best_model.pth'
...
model.eval()
print(f"loaded [{OPTION}] fine-tuned checkpoint:", CKPT)
...
def run_racing_eval(gt_path, out_json_name, label):
    ...
    print(f"\n=== [{OPTION}] {label} ({len(gt)} images, quantitative on non-AMBIGUOUS, n={len(quant)}) ===")
    print("accuracy:", acc)
...
racing_metrics_49 = run_racing_eval(
    ...
    f'racing_eval_49_results_{OPTION}.json',
...
racing_metrics_67 = run_racing_eval(
    ...
    f'racing_eval_67_results_{OPTION}.json',
...
print(f"\n=== [{OPTION}] RSCD 305-IMAGE TEST SET ===")
...
with open(f'{EXP_DIR}/rscd_test_eval_results_{OPTION}.json', 'w') as f:
    json.dump(rscd_metrics, f, indent=2)
print(f"\nsaved {EXP_DIR}/rscd_test_eval_results_{OPTION}.json")

print(f"\n\n=== [{OPTION}] SUMMARY ===")
print("49-img eval: acc=%.4f macroF1=%.4f DRY_recall=%.4f WET_recall=%.4f false_wet_rate=%s" % (
    racing_metrics_49['accuracy'], racing_metrics_49['macro_f1'], racing_metrics_49['dry_recall'], racing_metrics_49['wet_recall'],
    str(racing_metrics_49['false_wet_rate'])))
